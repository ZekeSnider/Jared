...
class EpicModule: RoutingModule {
    ...
    required init() {
        let fuccboi = Route(name: "/fuccboi", comparisons: [.StartsWith: ["/fuccboi"]], call: self.youreFuccboi, description: "Call Jared a fuccboi")
        let tip = Route(name: "/tip",comparisons: [.StartsWith: ["/tip"]], call: self.tipBuddy, description: "Tip your fedora to a fellow gentlesir")
        let bazinga = Route(name: "/bazinga", comparisons: [.StartsWith: ["/bazinga"]], call: self.sendBazing, description: "Become epic like sheldon cooper")
        let nice = Route(name: "/nice", comparisons: [.StartsWith: ["/nice"]], call: self.sendNice, description: "Congratulate someone on an epic accomplishment")
        let stop = Route(name: "stop", comparisons: [.Contains: ["stop"]], call: self.dontStop, description: "Stop Jared")
        let hello = Route(name: "hello", comparisons: [.Contains: ["hello"]], call: self.sendHello, description: "Say hello to Jared")
        let rate = Route(name: "/10", comparisons: [.StartsWith: ["/10"]], call: self.rateMessage, description: "Rate out of 10")
        let bigBunny = Route(name: "Big bunny", comparisons: [.Contains: ["Hey jared please send me the big buck bunny movie trailer"]], call: self.bigBunny, description: "Send the big buck bunny movie trailer")
        let slots = Route(name: "/slots", comparisons: [.StartsWith: ["/slots"]], call: self.spinSlots, description: "Play slots")
        let kill = Route(name: "/kill", comparisons: [.StartsWith: ["/kill"]], call: self.killJared, description: "Kill Jared")
        let clear = Route(name: "/clear", comparisons: [.StartsWith: ["/clear"]], call: self.clearChat, description: "Clear the chat")
        
        let kin = Route(name: "/kin", comparisons: [.StartsWith: ["/kin"]], call: self.getKin, description: "Discover your other-kin")
        let correction = Route(name: "Text Correction", comparisons: [.StartsWith: ["*"]], call: self.correct, description: "Handy text correction utility")
        let done = Route(name: "/done", comparisons: [.StartsWith: ["/done"]], call: self.donezo, description: "Declare your completion of a task")
        let choose = Route(name: "/choose", comparisons: [.StartsWith: ["/choose"]], call: self.makeChoice, description: "Make a tough decision easily")
        let guess = Route(name: "/guess", comparisons: [.StartsWith: ["/guess"]], call: self.guessNumber, description: "Fun Number guessing game")
        let face = Route(name: ":^)",comparisons: [.StartsWith: ["/grin",], .Contains: [":^)"]], call: self.printFace, description: "Send a shit eating grin")
        let muslim = Route(name: "Is Jalen Muslim?", comparisons: [.StartsWith: ["/isjalenmuslim"], .Contains: ["Is Jalen Muslim"]], call: self.isJalenMuslim, description: "Check if Jalen is a muslim")
        let jew = Route(name: "/jew", comparisons: [.StartsWith: ["/jew"]], call: self.printJew, description: "Jew")
