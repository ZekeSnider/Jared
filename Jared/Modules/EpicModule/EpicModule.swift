...
struct EpicModule: RoutingModule {
    ...
    init() {
        ...
        let kin = Route(comparisons: [.StartsWith: "/kin"], call: self.getKin)
        let correction = Route(comparisons: [.StartsWith: "*"], call: self.correct)
        let done = Route(comparisons: [.StartsWith: "/done"], call: self.donezo)
        let choose = Route(comparisons: [.StartsWith: "/choose"], call: self.makeChoice)
        let guess = Route(comparisons: [.StartsWith: "/guess"], call: self.guessNumber)
        let jew = Route(comparisons: [.StartsWith: "/jew"], call: self.printJew)
            ...
            if theInput == "new" {
                ...
                if guesses > -1{
                    ...
                    if theNumber > secretNum {
                        ...
                        SendText("OMG \(myRoom.buddyName) Wins! :^)", toRoom: myRoom)
                        SendText("It took you \(guesses) guesses", toRoom: myRoom)
                        guesses = -1
                    }
                }
                else {
